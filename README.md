...
Everything under `/api/v1/` is versioned by `API_VERSION` in `server/freshr/settings.py`. `admin/`, `docs/`, `redocs/` and `schema/` sit deliberately *outside* `/api/` — nginx proxies only `/api/` to Django in production, so those routes are reachable in local development only.
