...
import FilesColumn, { type FileUploadState } from "../components/notebook/FilesColumn";
import ChatColumn from "../components/notebook/ChatColumn";
import OptionsColumn, { type ActiveView } from "../components/notebook/OptionsColumn";
...
export default function NotebookPage() {
  ...
  const quizSessions = useQuizSessions(
    ...
    (msg) => { setPendingChatInput(msg); setActiveView("chat"); },
    () => setUpgradeModal(notebookArchivedModal),
  );
  const presentationSession = usePresentationSessions(
    ...
    () => setUpgradeModal({
      ...
      description: "You've hit your daily presentation limit on the free plan. Upgrade to Pro for unlimited presentations, more storage, and additional notebooks.",
    }),
    () => setUpgradeModal(notebookArchivedModal),
  );
  ...
  useEffect(() => {
    ...
    accountService
      .getAccountUsage()
      .then((usage) => { if (!cancelled) setAudioFeatureEnabled(usage.features?.audio_notes ?? false); })
      .catch(() => { if (!cancelled) setAudioFeatureEnabled(false); });
  ...
  useEffect(() => {
    async function loadNotebooksAndFiles() {
      try {
        ...
        if (!foundNotebookById) { setNotebookNotFound(true); return; }
  ...
  async function handleUpload(uploaded: File[]) {
        ...
        try {
          ...
          if (!response.success) {
            ...
            setUploadProgress((prev) =>
              prev.map((p, idx) => (idx === i ? { ...p, status: "error", error: errMsg } : p)),
            );
    ...
    if (successCount > 0) {
      ...
      try { setFiles(await notebookService.listFiles(notebookId)); }
      catch (err) { console.error(`[NotebookPage.tsx] Error while listing files for notebook ID: ${notebookId}, error: ${err}`); }
    }

    setTimeout(() => setUploadProgress((prev) => prev.filter((p) => p.status === "error")), 2000);
    setTimeout(() => setUploadProgress([]), 6000);
  }

  async function handleDeleteSelectedFiles(ids: string[]) {
    try {
      await Promise.all(ids.map((id) => notebookService.deleteFile(notebookId, id)));
      setFiles((prev) => prev.filter((f) => !ids.includes(f.id)));
      showToast(`${ids.length} file${ids.length > 1 ? "s" : ""} deleted`, "danger");
    } catch (err) {
    ...
    if (activeView === "audio") {
      ...
      const showAudioUpgrade = () =>
        setUpgradeModal({
          ...
          description: "Upload lecture recordings, get accurate Bangla + English transcripts, and turn them into structured study notes. Upgrade to Pro to unlock Audio Notes.",
        });
      const interceptPaidOnly = <T,>(promise: Promise<T>): Promise<T> =>
        promise.catch((err) => {
          const code = (err as any)?.response?.data?.code;
          if (code === "paid_only_feature") { setAudioFeatureEnabled(false); showAudioUpgrade(); }
          throw err;
        });
      ...
      async function pollAudioTranscript(
        transcriptId: string,
        isDone: (d: Awaited<ReturnType<typeof transcriptionService.getAudioTranscript>>) => boolean,
      ) {
        const deadline = Date.now() + POLL_TIMEOUT_MS;
        while (true) {
          const detail = await transcriptionService.getAudioTranscript(notebookId, transcriptId);
          if (isDone(detail)) return detail;
          if (Date.now() > deadline) throw new Error("Still running — check back in History in a few minutes.");
      ...
      return (
        <AudioColumn
          ...
          onTranscribeAudio={async (file, title) => {
            const kickoff = await interceptPaidOnly(transcriptionService.transcribeAudio(notebookId, file, title));
            const detail = await pollAudioTranscript(
              kickoff.transcript_id,
              (d) => d.transcription_status === "ready" || d.transcription_status === "failed",
            );
            if (detail.transcription_status === "failed") throw new Error(detail.transcription_error || "Transcription failed.");
            return { transcript_id: kickoff.transcript_id, transcript: detail.transcript_text };
          }}
          onGenerateNotes={async (transcriptId) => {
            await interceptPaidOnly(transcriptionService.generateNotesFromTranscript(notebookId, transcriptId));
            const detail = await pollAudioTranscript(
              transcriptId,
          ...
          onUpdateTranscript={(transcriptId, fields) =>
            interceptPaidOnly(transcriptionService.updateAudioTranscript(notebookId, transcriptId, fields))
          }
  ...
  return (
    <div className="flex flex-col h-dvh overflow-hidden">
      ...
      {notebook.is_archived && (
        <div className="flex items-center justify-center gap-3 px-6 py-2 bg-amber-500/10 border-b border-amber-500/20 shrink-0">
      ...
      {/* Top bar */}
      <div className="h-14 flex items-center justify-between px-6 border-b border-border bg-card shrink-0 gap-3">
        {isCompact ? (
          <button
          ...
          <button
            ...
            className="text-xs text-muted-foreground hover:text-foreground hover:underline underline-offset-4 transition-colors shrink-0"
          >
            ← Back to dashboard
          </button>
        )}
        ...
        <div className="flex-1 flex justify-center min-w-0">
          <NotebookTitle title={notebook.title} onSave={handleNotebookTitleSave} />
        </div>
      ...
      <div
        ...
        style={{ display: "grid", gridTemplateColumns: isCompact ? "1fr" : "220px 1fr 260px" }}
      >
        {!isCompact && (
      ...
      {isCompact && (
        <>
          <MobileDrawer open={toolsDrawerOpen} onClose={() => setToolsDrawerOpen(false)} side="left" width="min(240px, 85vw)" ariaLabel="Notebook tools">
            <OptionsColumn
              activeView={activeView}
              onViewChange={(v) => { setActiveView(v); setToolsDrawerOpen(false); }}
            />
          </MobileDrawer>

          <MobileDrawer open={filesDrawerOpen} onClose={() => setFilesDrawerOpen(false)} side="right" width="min(320px, 90vw)" ariaLabel="Files panel">
            <div className="h-full flex flex-col">{renderRightPanel()}</div>
          </MobileDrawer>
        </>
      )}
